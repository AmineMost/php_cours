...
A) Elle exécute un bloc de code si la condition précédente est fausse.
B) Elle exécute un bloc de code si la condition précédente est vraie. X
...
B) == compare les types, === compare uniquement les valeurs.X
C) == est utilisé dans les switch, === dans les if.
...
A) Il arrête l'exécution de la fonction actuelle et retourne une valeur.
B) Il continue l'exécution du script.
C) Il redémarre la boucle.
D) Il arrête le script entier. X
...
A) Exécuter un bloc de code lorsque aucun autre cas ne correspond. X
...
B) Elle est ignorée par le compilateur. X
